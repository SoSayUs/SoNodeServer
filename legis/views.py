...
def house_or_senate_hansards_view(request, region):
    ...
    prnt('-house_or_senate_hansards_view',style)
    ...
    if request.method == 'POST':
        ...
        subtitle = date
        view = None
        meetings = Meeting.objects.filter(Country_obj__id=country_dict['id'], meeting_type='Debate', DateTime__gte=date, DateTime__lt=date + datetime.timedelta(days=1)).filter(Chamber__in=current_chamber_list).values('id')
        posts = Post.objects.filter(id__in=[get_post_id(m['id']) for m in meetings]).order_by('-DateTime').prefetch_related(GenericPrefetch('Pointer_obj',[Meeting.objects.all()]))
        ...
        if view == 'Current':
            meetings = Meeting.objects.filter(Country_obj__id=country_dict['id'], meeting_type='Debate').filter(Chamber__in=current_chamber_list).values('id')
            posts = Post.objects.filter(id__in=[get_post_id(m['id']) for m in meetings]).order_by('-DateTime').prefetch_related(GenericPrefetch('Pointer_obj',[Meeting.objects.all()]))
...
def debate_view(request, region, chamber, govNumber, session, iden, year, month, day, hour, minute):
    ...
    context['debateUpdate'] = meetingUpdate
    context['debate'] = m
    context['speaker_id'] = speaker_id
    context['time'] = time
    context['id'] = id
    context['topic'] = topic
    context['topicList'] = [topic]
    return render_view(request, context, country_dict, 'legis')
...
def bill_view(request, region, chamber, govNumber, session, numcode):
    ...
    context = default_context(request, posts, 'bill_view', nav_options, sort='new', view='Overview')
    context['feed_title'] = title
    context['title_link'] = title_link
    context['billText'] = billText
    context['post'] = billPost
    return render_view(request, context, country_dict, 'legis')
...
def bills_view(request, region):
    ...
    if date:
        ...
        if sort == 'For You':
            ...
            if hasText.lower() == 'true':
                posts = Post.objects.filter(pointerType='Bill', Region_obj__id=country_dict['id'], filters__Chamber__in=current_chamber_list).filter(filters__contains={'has_text': True}).order_by(ordering, '-DateTime')
            elif hasText.lower() == 'false':
                posts = Post.objects.filter(pointerType='Bill', Region_obj__id=country_dict['id'], filters__Chamber__in=current_chamber_list).exclude(filters__contains={'has_text': True}).order_by(ordering, '-DateTime')
            else:
                posts = Post.objects.filter(pointerType='Bill').filter(Region_obj__id=country_dict['id'], filters__Chamber__in=current_chamber_list).order_by(ordering, '-DateTime')
            if title and current_chamber_name.lower() == 'all':
                title = "Government Bills"
            elif title:
                title = '%s Bills' %(current_chamber_name.replace('-', ' '))
    ...
    context = default_context(request, posts, 'bills_list', nav_options)
    context['feed_title'] = title
...
def officials_list(request, region):
    ...
    context = default_context(request, posts, 'rep_list', nav_options, sort='All', view='Current')
    context['feed_title'] = title
    context['extra_data'] = fetch_updated_objs(context['feed_list'], ['Party', 'District', 'ProvState'])
    return render_view(request, context, country_dict, 'legis')
...
def representative_view(request, region, name, iden):
    ...
    context = default_context(request, posts, 'representative_view', nav_options, sort='new')
    context['feed_title'] = title
    context['personTerms'] = termsList
    context['match'] = match_percentage
    context['voteMatches'] = vote_matches
    context['totalMatches'] = total_matches
    context['myVotes'] = my_votes
    context['topicList'] = [topic]
    return render_view(request, context, country_dict, 'legis')
